...
description: Parse and import bank statement PDFs from Philippine banks (GCash, BPI, Maya). Use when user mentions parsing, importing, uploading, adding, or processing bank statements, PDFs, or transactions from files. Automatically detects bank type and extracts transactions to local database.
allowed-tools: Bash, Read, Glob
---

# Parse Bank Statements

Import transactions from PDF bank statements into the local analyze-fin database.
...
BPI passwords typically follow: `SURNAME` + last 4 digits (e.g., `GARCIA1234`)
...
**With password (BPI):**
```bash
cd /Users/agallentes/git/analyze-fin && uv run analyze-fin parse path/to/bpi_statement.pdf --password SURNAME1234
```
...
Your transactions are now in the database. What would you like to know about your spending?
```
...
This looks like a BPI statement. BPI statements require a password (usually your SURNAME + last 4 digits).
...
Total: 92 transactions imported across 3 accounts

The Maya statement had a medium confidence score. Would you like to review those transactions?
```
...
**Wrong password:**
```
The password didn't work. BPI passwords are usually:
SURNAME + last 4 digits (e.g., GARCIA1234)
...
Suggest next steps:
- "Want to see a breakdown by category?"
- "Would you like to check for duplicates?"
- "Curious how much you spent this month?"
...
4. **Suggest next actions** - Guide the user through their workflow
